...
if(!skip | RECALCULATEPOPULATIONS) {
  ...
  if(file.exists("./inputs/KCChildPops.csv")) {
    ...
    KCPops <- get_population(geo_type = "blk", kingco = T, year = 2022, ages = 0:15)
...
ggplot(DT) +
  geom_histogram(alpha = 0.6, position = "identity", aes(x = childPopulationHalfMile, fill = Opportunity_Area))
...
Need to know which target areas are playgrounds
...
ggplot(DT) +
  geom_histogram(alpha = 0.6, position = "identity", aes(x = populationHalfMile, fill = Park_Category))
...
ggplot(DT) +
  geom_histogram(alpha = 0.6, position = "identity", aes(x = childPopulationHalfMile, fill = Park_Category))
...
Use all ages daily average

playground target area probably needed
