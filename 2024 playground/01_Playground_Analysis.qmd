...
-   Aggregates all playground within the park (effectively calculated as one large playground)
...
DTplot <- DT[, .(users = sum(num_child_prim, num_child_snd, num_child_spec, num_teen_prim, num_teen_snd, num_teen_spec, num_adult_prim, num_adult_snd, num_adult_spec, num_senior_prim, num_senior_snd, num_senior_prim, na.rm = TRUE)), by = .(datePOSIX, Park_Category_Local, park_name_full, playground)][, max := lapply(.SD, sum, na.rm = TRUE), .SDcols = c("users") ,by = .( Park_Category_Local, park_name_full)][playground == 1, popularity := users/max][,lapply(.SD, mean, na.rm = TRUE), .SDcols = c("popularity") ,by = .( Park_Category_Local, park_name_full)]

ggplot(DTplot) +
  geom_bar(stat = "identity", position = "stack", aes(y = park_name_full, x = popularity)) +
  facet_wrap(~Park_Category_Local, scales = "free_y") +
  xlab("Average Playground Popularity") +
  ylab("Park Name") +
...
note, there is a fair amount of missingness for this

```{r}
DT <- copy(ParkFull)

DTplot <- DT[, .(avg_Daily_Children = sum(num_child_prim, num_child_snd, num_child_spec, na.rm = TRUE)), by = .(datePOSIX, Elements)][,lapply(.SD, mean, na.rm = TRUE), .SDcols = c("avg_Daily_Children") ,by = .(Elements)]

ggplot(DTplot) +
  geom_bar(aes(weight = avg_Daily_Children, x = Elements))
...
DTplot <- DT[, .(max_Daily_Children = sum(num_child_prim, num_child_snd, num_child_spec, na.rm = TRUE)), by = .(datePOSIX, Elements)][,lapply(.SD, max, na.rm = TRUE), .SDcols = c("max_Daily_Children") ,by = .(Elements)]

ggplot(DTplot) +
  geom_bar(aes(weight = max_Daily_Children, x = Elements))

```

### Chart of Average Playground Popularity, sorted by Play Elements

Average of all observations for the percentage of observed users in the park who are located in the playground area. Include only observations where park is occupied.
...
DTplot <- DT[, .(avg_Daily_Children = sum(num_child_prim, num_child_snd, num_child_spec, na.rm = TRUE)), by = .(datePOSIX, SportsField, Park_Category, park_name_full)][,lapply(.SD, mean, na.rm = TRUE), .SDcols = c("avg_Daily_Children") ,by = .(SportsField, Park_Category, park_name_full)]

ggplot(DTplot[Park_Category == "Community"]) +
  geom_bar(position = "dodge",aes(weight = avg_Daily_Children, y = park_name_full, fill = SportsField))
...
thinking thorugh this.... latice of the above?
...
DTplot <- DT[, .(avg_Daily_Children = sum(num_child_prim, num_child_snd, num_child_spec, na.rm = TRUE)), by = .(datePOSIX, WalkScore)][,lapply(.SD, mean, na.rm = TRUE), .SDcols = c("avg_Daily_Children") ,by = .(WalkScore)]

ggplot(DTplot) +
  geom_point(aes(y = avg_Daily_Children, x = WalkScore))
...
DTplot <- DT[, .(avg_Daily_Children = sum(num_child_prim, num_child_snd, num_child_spec, na.rm = TRUE)), by = .(datePOSIX, TransitScore)][,lapply(.SD, mean, na.rm = TRUE), .SDcols = c("avg_Daily_Children") ,by = .(TransitScore)]

ggplot(DTplot) +
  geom_point(aes(y = avg_Daily_Children, x = TransitScore))
