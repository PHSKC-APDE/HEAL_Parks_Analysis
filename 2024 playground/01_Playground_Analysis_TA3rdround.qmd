...
- ensure all charts have playground use on x axis and not as gradient
- ratio of Median PARK use to area catchment
- ratio of Median PLAYGROUND use to the area catchment
...
DTplot <- DT[, .(med_Daily_Users = sum(num_child_prim, num_child_snd, num_child_tert, num_child_quat, num_teen_prim, num_teen_snd, num_teen_tert, num_teen_quat, num_adult_prim, num_adult_snd, num_adult_tert, num_adult_quat, num_senior_prim, num_senior_snd, num_senior_tert, num_senior_quat, na.rm = TRUE)), by = .(datePOSIX, populationHalfMile, park_name)][,lapply(.SD, median, na.rm = TRUE), .SDcols = c("med_Daily_Users") ,by = .( populationHalfMile, park_name)][, `Catchment Ratio` := populationHalfMile/med_Daily_Users]



ggplot(DTplot) +
  geom_bar(stat = "identity", position = "stack", aes(y = reorder(park_name,`Catchment Ratio`), x = `Catchment Ratio`)) +
