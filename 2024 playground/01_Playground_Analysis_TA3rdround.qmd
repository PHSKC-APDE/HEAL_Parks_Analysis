...
#clean old analysees, if any
unlink(here("2024 playground/outputs/charts"), recursive = T, force = T)
unlink(here("2024 playground/outputs/tables"), recursive = T, force = T)
unlink(here("2024 playground/outputs/maps"), recursive = T, force = T)


dir.create(here("2024 playground/outputs/charts"), recursive = TRUE, showWarnings = FALSE)
dir.create(here("2024 playground/outputs/maps"), recursive = TRUE, showWarnings = FALSE)
dir.create(here("2024 playground/outputs/tables"), recursive = TRUE, showWarnings = FALSE)
...
Manhattan Park was renovated in 2024. Because this analysis includes age of playground, we have split Manhattan park observations into two sets, one for 2023 and earlier, and the other of 2024 and later.

```{r splitting Manhattan park pre 2024}
ParkObservations[park_name == "Manhattan Park" , park_name := ifelse(studyDescription == "2024 annual study", "Manhattan Park 2024", "Manhattan Park 2023")]

ParksActivities[park_name == "Manhattan Park" , park_name := ifelse(studyDescription == "2024 annual study", "Manhattan Park 2024", "Manhattan Park 2023")]
...
```{r make or cleanup output directories}
dir.create(here("2024 playground/outputs/charts"), recursive = TRUE, showWarnings = FALSE)
dir.create(here("2024 playground/outputs/maps"), recursive = TRUE, showWarnings = FALSE)
dir.create(here("2024 playground/outputs/tables"), recursive = TRUE, showWarnings = FALSE)

unlink(here("2024 playground/outputs/charts/*"))
unlink(here("2024 playground/tables/*"))
unlink(here("2024 playground/maps/*"))
...
#DTplot[,Opportunity_Area := ifelse(Opportunity_Area == "No",  "Not Opportunity Area","Opportunity Area")]
DTplot[, old_med_Daily_Users := med_Daily_Users]
DTplot[, usersLabel := ifelse(`med_Daily_Users` > 200, paste0("~",round(med_Daily_Users,0)), "")]
DTplot[med_Daily_Users > 200, med_Daily_Users := 200]


ggplot(DTplot, aes(y = reorder(park_name, old_med_Daily_Users), x = med_Daily_Users)) +
  ...
  scale_x_continuous(labels = scales::comma) +
  theme(plot.title = element_text(size = TITLESIZE), plot.subtitle = element_text(size = SUBTITLESIZE))+
  geom_text(aes(label = usersLabel), vjust = 0.4, hjust = 1.2, size = 4, color = "white" )
...
DTplot <- DT[playground == 1, .(med_Daily_Users = sum(num_child_prim, num_child_snd, num_child_tert, num_child_quat, num_teen_prim, num_teen_snd, num_teen_tert, num_teen_quat, num_adult_prim, num_adult_snd, num_adult_tert, num_adult_quat, num_senior_prim, num_senior_snd, num_senior_tert, num_senior_quat, na.rm = TRUE)), by = .(datePOSIX, park_name)][,lapply(.SD, mean, na.rm = TRUE), .SDcols = c("med_Daily_Users") ,by = .(park_name)][order(med_Daily_Users),]
...
ggsave("./outputs/charts/MedianPlaygroundUse.png", width = 10, height = 11, units = c("in"))
```
...
DTplot <- DT[playground == 1, .(max_Daily_Users = sum(num_child_prim, num_child_snd, num_child_tert, num_child_quat, num_teen_prim, num_teen_snd, num_teen_tert, num_teen_quat, num_adult_prim, num_adult_snd, num_adult_tert, num_adult_quat, num_senior_prim, num_senior_snd, num_senior_tert, num_senior_quat, na.rm = TRUE)), by = .(datePOSIX, park_name)][,lapply(.SD, mean, na.rm = TRUE), .SDcols = c("max_Daily_Users") ,by = .(park_name)][order(max_Daily_Users),]
...
ggplot(DTplot, aes(y =  reorder(park_name, old_max_Daily_Users), x = max_Daily_Users)) +
  ...
  ylab("Park Name") +
  labs(title = "Maximum Daily Number of Playground Users") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  scale_y_discrete(limits = rev) +
  scale_x_continuous(labels = scales::comma) +
  theme(plot.title = element_text(size = TITLESIZE), plot.subtitle = element_text(size = SUBTITLESIZE))+
...
ggsave("./outputs/charts/MaxPlaygroundUse.png", width = 10, height = 11, units = c("in"))
```
...
ggplot(DTplot, aes(y = reorder(park_name,`Catchment Ratio`), x = `Catchment Ratio`)) +
  geom_bar(stat = "identity", position = "stack") +
  xlab("Ratio of Median Daily Park Users and 1/2 Mile Catchment") +
  ylab("Park Name") +
  labs(title = "Ratio of Median Daily Park Users \nand 1/2 Mile Catchment") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  scale_y_discrete(limits = rev) +
  scale_x_continuous(labels = scales::comma) +
  theme(plot.title = element_text(size = TITLESIZE), plot.subtitle = element_text(size = SUBTITLESIZE)) +
...
ggplot(DTplot, aes(y = reorder(park_name,`Catchment Ratio`), x = `Catchment Ratio`)) +
  geom_bar(stat = "identity", position = "stack") +
  xlab("Ratio of Median Daily Playground Users and 1/2 Mile Catchment") +
  ylab("Park Name") +
  labs(title = "Ratio of Median Daily Playground Users \nand 1/2 Mile Catchment") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  scale_y_discrete(limits = rev) +
  scale_x_continuous(labels = scales::comma) +
  theme(plot.title = element_text(size = TITLESIZE), plot.subtitle = element_text(size = SUBTITLESIZE)) +
...
To view interactive versions of the below maps, please see "interactive map of park and opportunity areas.html" and "interactive map of park and opportunity areas with labels.html" in the maps folder.
...
leafletvis <-leaflet() %>% addTiles() %>%
      setView(lat = 47.35, lng = -122.23, zoom = 10) %>%
      addPolygons(data = KCOpportunityAreaLEAF, color = "blue", stroke = 0, opacity = 1) %>%
      addMarkers(data =unique.data.frame(ParkFull[,.(lng = Longitude, lat =  Latitude, park_name)]), label = ~park_name, labelOptions = labelOptions(noHide = FALSE, textsize = "10px", textOnly = F ,opacity = .65 ))

library(htmlwidgets) 

#dir.create(here("2024 playground/outputs/maps/"), recursive = TRUE, showWarnings = FALSE)
saveWidget(leafletvis,file = here("2024 playground/outputs/maps/interactive map of park and opportunity areas.html"), selfcontained = TRUE)

leafletvislabels <-leaflet() %>% addTiles() %>%
      setView(lat = 47.35, lng = -122.23, zoom = 10) %>%
      addPolygons(data = KCOpportunityAreaLEAF, color = "blue", stroke = 0, opacity = 1) %>%
...
#dir.create(here("2024 playground/outputs/maps/"), recursive = TRUE, showWarnings = FALSE)
saveWidget(leafletvislabels,file = here("2024 playground/outputs/maps/interactive map of park and opportunity areas with labels.html"), selfcontained = TRUE)
```
...
leaflet() %>% addTiles() %>%
      setView(lat = 47.35, lng = -122.23, zoom = 10) %>%
      addPolygons(data = KCOpportunityAreaLEAF, color = "blue", stroke = 0, opacity = 1) %>%
      addMarkers(data =unique.data.frame(ParkFull[,.(lng = Longitude, lat =  Latitude, park_name)]), label = ~park_name)

```

\newpage
![Full View](./inputs/map full.png)
