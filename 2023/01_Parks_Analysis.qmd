...
pacman::p_load(sf, #create spatial objects around long and lat
               kcparcelpop,
...
The following metrics rely on counts of people observed. The underlying data are of people observed within an observation period (e.g. "morning1"). It is possible that the same people may be observed across multiple blocks of time and multiple target areas.
...
DT <- SOPARCAggregated[order(park_name_full),.("Number of Attendees" = sum(num_child_prim, num_child_snd, num_child_spec, num_teen_prim, num_teen_snd, num_teen_spec, num_adult_prim, num_adult_snd, num_adult_spec, num_senior_prim, num_senior_snd, num_senior_spec, na.rm = TRUE)), by = .("Park Name" = park_name_full)]
...
### Average Park Users By Time Period (not reported)
...
for(park in unique(SOPARCAggregated$park_name_full)) {
  ...
  geom_text(aes(label = `Users`, colour = "#FFFFFF"), vjust = 01.50, ) +
...
- These are technically "person time of use" and a measure of how much each target area is used in a time period.
...
for(park in unique(SOPARCAggregated$park_name_full)) {
  ...
  geom_treemap_text(color = "white",
                    place = "centre",
                    size = 6) +
  labs(title = paste0(park, "\n", cityName,"\n","2023"), subtitle = paste("Rate of Park Use Per Period")) +
  theme(plot.title = element_text(size = 12),
        plot.subtitle = element_text(size = 14,),
        axis.title.x = element_text(size = 12))
...
### Distribution Of Park Users By Ages (not reported)
...
for(park in unique(SOPARCAggregated$park_name_full)) {
  ...
  geom_col()  +
  labs(title = paste0(park,"\n", cityName,"\n", "2023"), subtitle = paste("Average Users Per Age")) +
  geom_text(aes(label = `Users`, colour = "#FFFFFF"), vjust = 01.50, ) +
  scale_color_manual(values=c("#FFFFFF")) +
  scale_y_continuous(limits=c(0,topOfY),) +
  theme(plot.title = element_text(size = 12),
...
for(park in unique(SOPARCAggregated$park_name_full)) {
  ...
  geom_treemap_text(color = "white",
                    place = "centre",
                    size = 6) +
  labs(title = paste0(park, "\n", cityName,"\n","2023"), subtitle = paste("Rate of Park Use Per Age")) +
  theme(plot.title = element_text(size = 12),
        plot.subtitle = element_text(size = 14,),
        axis.title.x = element_text(size = 12))
...
This measure doesn't have associated charts

```{r analysis of occupancy rate}
#| output: true


DT <- SOPARCtoAggregate[order(park_name_full), ifelse(any(occupied %in% "Yes"), 1, 0), by = .(park_name_full, periodBasedOnSequence,dayNumberBasedOnSequence)][, .( "Rate of Periods Occupied" =sum(V1)/24 ),by=.("Park Name" = park_name_full)]
...
DT <- merge(SOPARCActivities, SOPARCtoAggregate[,.("record_id" = as.numeric(record_id), park_name_full)], all.x = T, by.x = "record_id_aggregated", by.y = "record_id")
...
```{r save workbook}
dir.create(here("./2023/outputs/tables/pivot_ready"), recursive = T)
