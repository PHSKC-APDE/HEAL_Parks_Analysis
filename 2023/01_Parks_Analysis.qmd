...
unique(SOPARCObservations[park_name == "Crystal Springs Park", datePOSIX])

unique(SOPARCObservations[park_name == "Puget Sound Park", datePOSIX])

unique(SOPARCObservations[park_name == "Mathison", datePOSIX])
...
## Assigning population counts

For each park we estimate the number of people within 1 mile. This relies on access to APDE population data and use of the RADS library.

words from Daniel
you take the XY and you buffer (`st_buffer`) it a mile

so now you have a polygon (in this case, a circle)


you then relate it to census blocks via spatagg::create_xwalk (you might have to do some data cleaning in between)

and then you use spatagg::crosswalk to convert get_population results at the block level to the buffered XY

and that'll give you an estimate/approximation of the population in the radii
...
ParkGeos <- st_buffer(ParkGeos, units::set_units(1, mile)) # units::set_units is intelligent about types of units so you can specify "mile". st_buffer will create a perimeter of the provided units.
#confirm that tihs peremeter defaults to a circle for points
plot(ParkGeos)


blk_shapes <- st_read("//dphcifs/APDE-CDIP/Shapefiles/Census_2020/block/kc_block.shp") 
blk_shapes <- st_transform(blk_shapes, st_crs("EPSG:4326")) #conform to same crs

plot(blk_shapes)

Park_blk_intersection <- st_intersection(ParkGeos, blk_shapes) #this take a looooong time. save and load in future

plot(Park_blk_intersection[2])

blk_pops <- get_population( ,geo_type = "blk")


st_point(unlist(ParkGeos[1,.(lon,lat)]))
ParkGeos <- st_as_sf(x = ParkGeos,coords = c("lon","lat"), dim = "XY")

st_sfc(ParkGeos, crs = 2926)
st_sfc(ParkGeos, crs = 2926)
st_transform(ParkGeos, 2926)


st_buffer(ParkGeos, units::set_units(1, mile))
?st_buffer
```
...
DT <- SOPARCtoAggregate[order(park_name), ifelse(any(occupied %in% "Yes"), 1, 0), by = .(park_name, periodBasedOnSequence,dayNumberBasedOnSequence)][, .( "Rate of time Occupied" =round((sum(V1)/24 )*100,0)),by=.("Park Name" = park_name)]
...
### Number of People Doing an activitiy (not reported)
...
### Rate of Activity Observed in Park (in progress)

This is a measure of, in how many observation periods, an activity was reported. This is across all areas and sub areas.

Still in progress (not ready)

```{r Rate of Activities Per Park}
#| output: true
...
SOPARCtoAggregate[,sum(num_child_spec, num_teen_spec, num_adult_spec, num_senior_spec, na.rm = T) , by = .(park_name, dayNumberBasedOnSequence,periodBasedOnSequence,activity = prim_act_name)][V1 > 0,])[,sum(V1), by = .(park_name, dayNumberBasedOnSequence,periodBasedOnSequence,activity)][order(park_name) ,.("Rate" = .N / 24), by = .("Park Name" = park_name,"Activity" = activity)]



WB$add_worksheet("Rate Activity")
WB$add_data(sheet = "Rate Activity", DT)
...
#save charts as individual files
#cleanup old images of this chart
if(file.exists(here("./2023/outputs/tables/rate_activity/"))) {
  unlink(here("./2023/outputs/tables/rate_activity/"), recursive = T)
}
dir.create(here("./2023/outputs/tables/rate_activity"), recursive = T)
...
for(park in unique(SOPARCtoAggregate$park_name)) {
  KBObject <- kbl(DT[`Park Name` == park,.(Activity, Rate = paste0(round(Rate * 100), "%"))][order(Activity)],
    ...
    save_kable(x = KBObject,file =  here(paste0("./2023/outputs/tables/rate_activity/",park,"rate_activity.pdf")))
  
  FT <- DT[`Park Name` == park,.(Activity, Rate = paste0(round(Rate * 100), "%"))][order(Activity)] %>% regulartable() %>% autofit()

  read_docx() %>%
  body_add_flextable(FT) %>%
  print(target = paste0("./outputs/tables/rate_activity/",park,"_rate_activity.docx"))  
...
- this is of time that the activity was observed, not

```{r save workbook}
dir.create(here("./2023/outputs/tables/pivot_ready"), recursive = T)
WB$save("./outputs/tables/pivot_ready/analysis_tables.xlsx", overwrite = TRUE)
```
