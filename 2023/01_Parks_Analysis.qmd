...
This document outlines procedures and tehcnical considerations for the 2023 analysis of data from the HEAL Parks Study.
...
For this analysis, we are not using sub areas. These can be collapsed into single areas. For each observation, sub areas will be collapsed using the following rules: numerical observations will be added together categorical become affirmative/existing if any of the subareas are affirmative/existing timestamp of the earliest observation in the set will be used

Sub areas of the same target area will be assumed to be of the same observation period based on the following logic: for a sequence of sub areas observed in the same 50 hours period apparantly missing sub-areas will be ignored (assumption: not all sub areas are necessary)
...
parkStats[,`:=`(strateg1exec = FALSE, 
             ...
             numberDaysObserved = ifelse(meetsExpected, 3, NA),
...
for(park in unique(SOPARCObservationsToFix$park_name)) {
  ...
  if(parkStats[park_name == park,]$numberDaysObserved == 3) { #what to do if there are the correct number of days
    for(dateToFix in stratResult[exact == FALSE,]$datePOSIX) { # per day
...
SOPARCObservationsCorrected <- copy(SOPARCObservationsToFix[park_name %in% parkStats[meetsExpected == TRUE, park_name],])

SOPARCObservationsOK <- rbind(SOPARCObservationsOK, SOPARCObservationsCorrected)
SOPARCObservationsToFix <- copy(SOPARCObservationsToFix[park_name %in% parkStatscheck[meetsExpected == FALSE, park_name],])
...
At this point rudementary timestamps have been assigned based on the expected times of observation. Now we apply a series of heuristics to correct obvious errors.
...
After cleaning and QA, data are saved to a csv file for review titled, "QA_manual_review.csv".

A separate table of QA checks is saved as well. This includes information about all parks, incldued or not. Note that if the USEGOODDATA flag is set, only parks that pass QA are included in analysis. titled "QA_metadata.csv".

```{r save data to review QA output}
#save for manual inspection
write.csv(PostQADT, "QA_manual_review.csv")
write.csv(parkStats, "QA_metadata.csv")
```
...
SOPARCAggregated <- SOPARCtoAggregate[,
                                      ...
                                      by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]
...
# SOPARCAggregated$num_child_prim <- SOPARCtoAggregate[, ceiling(mean(as.numeric(num_child_prim))), by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]$V1
# SOPARCAggregated$num_child_snd <- SOPARCtoAggregate[, ceiling(mean(as.numeric(num_child_snd))), by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]$V1
# SOPARCAggregated$num_child_spec <- SOPARCtoAggregate[, ceiling(mean(as.numeric(num_child_spec))), by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]$V1
# SOPARCAggregated$num_teen_prim <- SOPARCtoAggregate[, ceiling(mean(as.numeric(num_teen_prim))), by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]$V1
# SOPARCAggregated$num_teen_snd <- SOPARCtoAggregate[, ceiling(mean(as.numeric(num_teen_snd))), by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]$V1
# SOPARCAggregated$num_teen_spec <- SOPARCtoAggregate[, ceiling(mean(as.numeric(num_teen_spec))), by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]$V1
# # SOPARCAggregated$num_adult_prim <- SOPARCtoAggregate[, ceiling(mean(as.numeric(num_adult_prim))), by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]$V1
# # SOPARCAggregated$num_adult_snd <- SOPARCtoAggregate[, ceiling(mean(as.numeric(num_adult_snd))), by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]$V1
# SOPARCAggregated$num_adult_spec <- SOPARCtoAggregate[, ceiling(mean(as.numeric(num_adult_spec))), by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]$V1
# SOPARCAggregated$num_senior_prim <- SOPARCtoAggregate[, ceiling(mean(as.numeric(num_senior_prim))), by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]$V1
# SOPARCAggregated$num_senior_snd <- SOPARCtoAggregate[, ceiling(mean(as.numeric(num_senior_snd))), by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]$V1
# SOPARCAggregated$num_senior_spec <- SOPARCtoAggregate[, ceiling(mean(as.numeric(num_senior_spec))), by = .(park_name, tar_area, dayNumberBasedOnSequence, period)]$V1
# 
# 
...
kbl(SOPARCAggregated[order(park_name), .("Park Name" = unique(park_name))], caption = "Parks Included In Analysis")
...
kbl(SOPARCAggregated[order(park_name),.("Number of Attendees" = sum(num_child_prim, num_child_snd, num_child_spec, num_teen_prim, num_teen_snd, num_teen_spec, num_adult_prim, num_adult_snd, num_adult_spec, num_senior_prim, num_senior_snd, num_senior_spec, na.rm = TRUE)), by = .("Park Name" = park_name)], 
      caption =  "Total Number of Attendees",
    booktabs = T) %>% kable_styling(latex_options = "striped")
...
```{r Average Park Visitors by time}
#| output: true
...
```{r Rate of Park Use By Time}
#| output: true
kbl(SOPARCtoAggregate[order(park_name), sum(num_child_prim, num_child_snd, num_child_spec, num_teen_prim, num_teen_snd, num_teen_spec, num_adult_prim, num_adult_snd, num_adult_spec, num_senior_prim, num_senior_snd, num_senior_spec, na.rm = TRUE)/6 , by = .(park_name, period)][,.("Time Period" = period, "Proportion of Use" = paste0(round((V1/ sum(V1))*100,1), "%")), by = .("Park Name" = park_name) ],
    caption = "Rate of Park Use By Time Period",
    booktabs = T) %>% kable_styling(latex_options = "striped")

kbl(SOPARCAggregated[order(park_name), sum(num_child_prim, num_child_snd, num_child_spec, num_teen_prim, num_teen_snd, num_teen_spec, num_adult_prim, num_adult_snd, num_adult_spec, num_senior_prim, num_senior_snd, num_senior_spec, na.rm = TRUE)/3 , by = .(park_name, period)][,.("Time Period" = period, "Proportion of Use" = paste0(round((V1/ sum(V1))*100,1), "%")), by = .("Park Name" = park_name) ],
    caption = "Rate of Park Use By Time Period",
    booktabs = T) %>% kable_styling(latex_options = "striped")
```
Notes: Rate is within-park (each park totals to 100%).

## Visitor Characteristics/Demography

```{r Distribution of attendee ages}
#|output: true
kbl(SOPARCtoAggregate[order(park_name), .( "Age Group" = rep(c("child","teen","adult","senior")), 
                                           "Average Number of People" = round(c(sum(num_child_prim, num_child_snd, num_child_spec, na.rm = TRUE)/6,
                      sum(num_teen_prim, num_teen_snd, num_teen_spec, na.rm = TRUE)/6,
                      sum(num_adult_prim, num_adult_snd, num_adult_spec, na.rm = TRUE)/6,
                      sum(num_senior_prim, num_senior_snd, num_senior_spec, na.rm = TRUE)/6))), by = .("Park Name" = park_name)],
    caption = "Daily Average Number of Attendees by Age Group",
    booktabs = T)  %>% kable_styling(latex_options = "striped")

```

```{r rate of park use by age}
#|output: true
kbl(SOPARCtoAggregate[order(park_name), .("Age Group" = rep(c("child","teen","adult","senior")), 
                      "Percent Use" = paste0(round(c(sum(num_child_prim, num_child_snd, num_child_spec, na.rm = TRUE) / sum(num_child_prim, num_child_snd, num_child_spec, num_teen_prim, num_teen_snd, num_teen_spec, num_adult_prim, num_adult_snd, num_adult_spec, num_senior_prim, num_senior_snd, num_senior_spec, na.rm = TRUE),
...
Park Occupancy: percentage of the 24 observation periods where the park had any occupancy Doing this by target areas has problem because the content of target areas is not stable.

```{r analysis of occupancy}

SOPARCtoAggregate[order(park_name), ifelse(any(occupied %in% "Yes"), 1, 0), by = .(park_name, periodBasedOnSequence,dayNumberBasedOnSequence)][, .("Percent of time Occupied" =round((sum(V1)/24 )*100,0)),by=park_name]
...
Frequency of Activities: Counts of how often at least 1 person was doing which of the activities. Time as proportion of the 24 observation periods
