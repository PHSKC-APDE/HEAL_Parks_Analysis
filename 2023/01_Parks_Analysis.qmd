...
CHARTFILEFORMAT <- "pdf" #choose a file format for the exported files to use. Note, "png" for portability, or "pdf" for vectorized image are recommended
...
RemoveMe <- openxlsx2::read_xlsx("Records-to-Remove.xlsx")
...
if(file.exists(here("./2023/charts/avg_visitors_x_time/"))) {
  unlink(here("./2023/charts/avg_visitors_x_time/"), recursive = T)
}
dir.create(here("./2023/charts/avg_visitors_x_time"))
...
for(park in unique(SOPARCAggregated$park_name)) {
  ...
  ggsave(paste0(park,"_avg_visitors_x_time.", CHARTFILEFORMAT), plot = PTExport, path = paste0("./charts/avg_visitors_x_time"),width = CHARTWIDTH, height = CHARTHEIGHT)
}
...
#save charts as individual files
#cleanup old images of this chart
if(file.exists(here("./2023/charts/rate_visitors_x_time/"))) {
  unlink(here("./2023/charts/rate_visitors_x_time/"), recursive = T)
}
dir.create(here("./2023/charts/rate_visitors_x_time"))
...
for(park in unique(SOPARCAggregated$park_name)) {
  ...
  ggsave(paste0(park,"_rate_visitors_x_time.", CHARTFILEFORMAT), plot = PTExport, path = paste0("./charts/rate_visitors_x_time"),width = CHARTWIDTH, height = CHARTHEIGHT)
}
...
WB$save("analysis_data.xlsx", overwrite = TRUE)
```
