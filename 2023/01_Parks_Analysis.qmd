...
kbl(SOPARCtoAggregate[order(park_name), ifelse(any(occupied %in% "Yes"), 1, 0), by = .(park_name, periodBasedOnSequence,dayNumberBasedOnSequence)][, .( "Percent of time Occupied" =round((sum(V1)/24 )*100,0)),by=.("Park Name" = park_name)],
...
kbl(DT,
    ...
    booktabs = TRUE,
    longtable = TRUE) %>%collapse_rows(1:3, row_group_label_position = "stack") %>% 
...
WB$save("test.xlsx", overwrite = TRUE)
```
