...
for(park in parkStats[meetsExpected == "FAIL",]$park_name) {
  ...
  areasExpects <- parkStats[park_name == park,areaCount]
  ...
  if(parkStats[park_name == park,]$numberDaysObserved == 3 & parkStats[park_name == park,]$numberDaysComplete == 2) { #what to do if there are the correct number of days and only one day is not exact
    ...
    #if no target area has more than the expected, walk through the day in order, and insert missing observations
    if(all(SOPARCSingle[ datePOSIX == dateToFix, .N <= 8 , by = tar_area]$V1)) {
      areaScanner <- 1
      for(rowCounter in 1:nrow(SOPARCSingle)) {
        expectedTargetArea <- areaScanner
      }
    }
    test <- SOPARCSingle[ datePOSIX == dateToFix, ]
...
SOPARCSingle[datePOSIX == "2023-08-16",]
