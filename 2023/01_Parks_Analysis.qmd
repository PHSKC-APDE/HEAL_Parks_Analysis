...
#convert time to time data type and save as separate variable
RawSOPARCObservations[, timestampPOSIX := as.POSIXct(strptime(start_time, "%Y-%m-%d %H:%M"), "America/Los_Angeles") ]
```
...
for(park in unique(SOPARCObservations[,park_name])) {
  for(area in 1:max(SOPARCObservations[park_name == park, tar_area])) {
    ...
    if(all(SOPARCObservations[park_name == park & tar_area == area, sub_area ] == "")) { #testing areas where all observations are without sub areas
      if(!nrow(SOPARCObservations[park_name == park & tar_area == area,] ) == 24) {
        #check that there are 3 days and 8 observations per day
        if(length(unique(as.Date(SOPARCObservations[park_name == park & tar_area == area,]$timestampPOSIX, tz = "America/Los_Angeles"))) == 3){
          #checks for those that have 3 days...
          #confirm that there are 8 observations per day, and make corrections if not
          for(day in unique(as.Date(SOPARCObservations[park_name == park & tar_area == area,]$timestampPOSIX, tz = "America/Los_Angeles"))) {
            if(nrow(SOPARCObservations[park_name == park & tar_area == area & as.Date(timestampPOSIX, tz = "America/Los_Angeles") == day,]) > 8) {
              ...
              print(paste("there are too many observations in", park, area, as.Date(day)))
