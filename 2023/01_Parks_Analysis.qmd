...
if( !(file.exists(here("2023/inputs/Park Activities.csv"))) | REBUILDACTIVITYTABLE){
SOPARCActivities <- data.table("record_id" = SOPARCObservations[prim_act_name != "",]$record_id, 
                               ...
                               "adult" = 0, 
                               "senior" = 0)
for(id in SOPARCActivities$record_id) {
...
write.csv(SOPARCActivities, here("2023/outputs/data-metadata/Park Activities.csv"), row.names = FALSE)
write.csv(SOPARCActivities, here("2023/inputs/Park Activities.csv"), row.names = FALSE)
} else {
  SOPARCActivities <-  fread(here("2023/inputs/Park Activities.csv"))
}
```
...
In this step we also append changed record_ids to our activity table and save the updated activity table
...
#save newly udpated activity table
...
For each park we estimate the number of people within a half-mile. This is based on the generally accepted threshold of a 10-minute walk to a park as a metric goal (https://10minutewalk.org/). 
This relies on access to APDE population data and use of the RADS library.

Depends on park longitudes and latitudes provided in the meta data.

```{r create population estimates}
#this chunk is very slow, so if the data already exists, do not rerun.
skip <- FALSE
if(file.exists(here("2023/outputs/data-metadata/Park Populations.csv"))) {
  ParkPopulationTable <- fread(here("2023/outputs/data-metadata/Park Populations.csv"))
...
For the 2023 analysis we advised creating a study tracker. For each time a park is studied, a new entry is created. Each entry should includ the park name, the date the study staretd, and a brief description of the study. We generate and save a study tracker that covers 2022 and 2023 studies.
...
WBs <- sapply(unique(SOPARCAggregated[,park_name_full]), assign, value = wb_workbook())
```
...
Total number of users observed in the park across all observation periods.
...
Average number of users observed in the park each day

```{r vaerage number of daily users}
#| output: true
...
write.csv(DT, here("2023/outputs/tables/avg_users/num_users.csv"), row.names = FALSE)
...
This is a measure of the 3 day average number of users of a park within each time period.
...
for (i in names(WBs)) {
  wb = WBs[[i]]
  wb$add_worksheet("Avg Users x Period")
  wb$add_data(sheet = "Avg Users x Period", DT[`Park Name` == i])
}
...
### Rate of Daily Park Use By Time Period

This is a measure of the 3 day average number of users of a park within each time period.


This is a measure of the rate of people observed in a park over the study period.
...
for (i in names(WBs)) {
  wb = WBs[[i]]
  wb$add_worksheet("Rate Use x Period")
  wb$add_data(sheet = "Rate Use x Period", DT[`Park Name` == i])
}
...
for(park in unique(SOPARCAggregated$park_name_full)) {
  ...
  PTExport <-  ggplot(DT[`Park Name` %in% park], aes(x =`Park Name`, y = `Rate of Use`, fill = `Time Period`)) +
  geom_bar(position = "stack", stat = "identity") +
      #geom_text((aes(label = paste0(`Time Period`," \n \n",round(`Rate of Use`*100),"%")))) +
      #labs(title = paste0(park, " ", cityName," ","2023"), subtitle = paste("Rate of Park Use Per Period"))  +
      theme(plot.title = element_text(size = 8),
            plot.subtitle = element_text(size = 14,),
            axis.title.x = element_text(size = 12)) +
...
This is a measure of the daily average of users observed in each age group.
...
for (i in names(WBs)) {
  wb = WBs[[i]]
  wb$add_worksheet("Avg Users x Age")
  wb$add_data(sheet = "Avg Users x Age", DT[`Park Name` == i])
}
...
for (i in names(WBs)) {
  wb = WBs[[i]]
  wb$add_worksheet("Rate Use x Age")
  wb$add_data(sheet = "Rate Use x Age", DT[`Park Name` == i])
}
...
This is a measure of what percentage of observation periods had any target area occupied. 

For each park, the number of observation periods with any target area in occupied status is devided by the total number of observation periods (24)
...
for (i in names(WBs)) {
  wb = WBs[[i]]
  wb$add_worksheet("Rate Occupied")
  wb$add_data(sheet = "Rate Occupied", DT[`Park Name` == i])
}
...
This is the raw count of number of users observed doing an activity in a park over the study period
...
for (i in names(WBs)) {
  wb = WBs[[i]]
  wb$add_worksheet("Users per Activity")
  wb$add_data(sheet = "Users per Activity", DT[`Park Name` == i])
}
...
This is a measure of the rate of users engaged in an activity per park.

For each activity, the number of users engaged in that activity is divided by the total number of users observed in the study.
...
for (i in names(WBs)) {
  wb = WBs[[i]]
  wb$add_worksheet("Rate User Activity")
  wb$add_data(sheet = "Rate User Activity", DT[`Park Name` == i])
}
...
This is a measure of the rate of activities observed across all possible observation periods (24)
...
DT <- merge(SOPARCActivities, SOPARCtoAggregate[record_id != "X",.("record_id" = as.numeric(record_id), park_name_full)], all.x = T, by.x = "record_id_aggregated", by.y = "record_id")
...
for (i in names(WBs)) {
  wb = WBs[[i]]
  wb$add_worksheet("Rate Activity Observed")
  wb$add_data(sheet = "Rate Activity Observed", DT[`Park Name` == i])
}
...
### Ratio of use to catchment

For each park we calculate a ratio of how many people visit per day on average rleative to how many people live within 0.5 miles of the park.

This is provided per 1000 residents to improve readability.
...
DT <- DT[order(`Park Name`), .("Ratio" = Users / populationHalfMile), by = `Park Name`][, .("Ratio Per 1000" = Ratio *1000), by = "Park Name"]
...
for (i in names(WBs)) {
  wb = WBs[[i]]
  wb$add_worksheet("Ratio Use Catchment")
  wb$add_data(sheet = "ratio Use Catchment", DT[`Park Name` == i])
}
...
WB$save("./outputs/tables/pivot_ready/analysis_tables.xlsx")
...
```{r create workbooks specific to each park}

```

```{r save measure summary}
