...
This document outlines procedures and technical considerations for the 2023 analysis of data from the HEAL Parks Study.
...
Data are currently stored in ITHS REDCap (link). The project name is: <name>. In order for this script to operate correctly, you should request access to the project and then create an API key. This information goes into your secrets document.
...
Drop innacureate subareas | Some parks were identified as having having sub area data input without having multiple sub areas (and so shouldn't be processed as sub areas). The subarea entry for these parks is removed. 
...
* categorical observations become affirmative/existing if any of the subareas are affirmative/existing 
...
* check if the expected list of sub areas are in teh data set if too many, attempt to identify redundancies and remove these if too few, interpolate missing information
...
write.csv(parkStats, "QA_report.csv")
...
When executing this script, the use may choose to use all data, or only data that have passed QA. It is generally suggested to only use data that have passed QA.
...
parkStats$analysisReady <- FALSE
parkStats[park_name %in% passPeriodQA, analysisReady := TRUE]

#save for manual inspection and update QA report
write.csv(PostQADT, "temp_PreAnalysis_manual_review.csv")
write.csv(parkStats, "QA_report.csv")
write.csv(SOPARCAnalysisSet, "SOPARCDataPrepared.csv")
...
write.csv(SOPARCAggregated, "SOPARCDataPreparedAggregated.csv")
...
kbl(SOPARCAggregated[order(park_name),.("Attendees" = round(sum(num_child_prim, num_child_snd, num_child_spec, num_teen_prim, num_teen_snd, num_teen_spec, num_adult_prim, num_adult_snd, num_adult_spec, num_senior_prim, num_senior_snd, num_senior_spec, na.rm = TRUE)/3 ,0)), by = .("Park Name" = park_name, "Time Period" = period)],
    caption = "Daily Average Number of Attendees Per Time Period",
    booktabs = TRUE,
    longtable = TRUE) %>% kable_styling(latex_options = c("striped", "repeat_header", "hold_position"))
...
kbl(SOPARCAggregated[order(park_name), .( "Age Group" = rep(c("child","teen","adult","senior")), 
...
kbl(SOPARCtoAggregate[order(park_name), ifelse(any(occupied %in% "Yes"), 1, 0), by = .(park_name, periodBasedOnSequence,dayNumberBasedOnSequence)][, .("Percent of time Occupied" =round((sum(V1)/24 )*100,0)),by=park_name],
...
NOTES:
* activities are currently not properly handled for sub areas. Need to improve parsing for this to pass QA
* counts of people doing activities are unreliable as there may be double counting of individuals

```{r Number of people doing an activity in a park}
#| output: false

#this metric doesn't make sense given double counting of target areas
kbl(rbind(SOPARCtoAggregate[,sum(num_child_prim, num_teen_prim, num_adult_prim, num_senior_prim, na.rm = T) , by = .(park_name,activity = prim_act_name)][V1 > 0,],
...
kbl(rbind(SOPARCtoAggregate[,sum(num_child_prim, num_teen_prim, num_adult_prim, num_senior_prim, na.rm = T) , by = .(park_name, dayNumberBasedOnSequence,periodBasedOnSequence,activity = prim_act_name)][V1 > 0,],
