...
if( !(file.exists(here("2022/local_only/SOPARCAnalysisSet.csv"))) | REBUILDANALYSISSET){
      ...
      for(targetArea in unique(SOPARCAnalysisSet[park_name == park & datePOSIX == date, tar_area]))
        for(selected_period in unique(SOPARCAnalysisSet[park_name == park & datePOSIX == date & tar_area == targetArea, period])) {
          if(nrow(SOPARCAnalysisSet[park_name == park & datePOSIX == date & tar_area == targetArea & period == selected_period,]) != 2) {
            ...
            #write.csv( SOPARCAnalysisSet[park_name == park & datePOSIX == date & tar_area == targetArea & period == selected_period,], paste0(here("2022/outputs/tofix/BROKEN_"),park,"_","",".csv"))
          }
        }
    }
  ...
  write.csv(SOPARCActivities, here("2022/local_only/SOPARCAnalysisSet.csv"), row.names = FALSE)
  write.csv(SOPARCActivities, here("2022/outputs/data-metadata/SOPARCAnalysisSet.csv"), row.names = FALSE)
} else {
  SOPARCActivities <-  fread(here("2022/local_only/SOPARCAnalysisSet.csv"))
}
```
