...
SOPARCAnalysisSet dayNumberBasedOnSequence
...
ParksMetaData <- as.data.table(read_xlsx(here("2024/inputs/Parks_MASTER.xlsx"), sheet = "Sheet1"))
```
...
#time period
SOPARC2023$time_period <- ""
SOPARC2023[,paste(toupper(substr(periodBasedOnSequence, 1, 1)), substr(periodBasedOnSequence, 2, nchar(periodBasedOnSequence)-1), sep="")]

names(SOPARCAnalysisSet)[!(names(SOPARCAnalysisSet) %in% names(SOPARC2023))]
names(SOPARC2023)
names(SOPARCAnalysisSet)
names(SOPARC2023)[!(names(SOPARC2023) %in% names(SOPARCAnalysisSet))]
SOPARC2023[,c("period", "Park Address", "City", "Zip", "Neighborhood", "Images", "Tract", "Latitude", "Longitude", "Equity Score Priority ", "Planned park changes?", "Notes", "censusYear","populationHalfMile"):=NULL]
...
substr(SOPARC2023$periodBasedOnSequence, nchar(SOPARC2023$periodBasedOnSequence)-1, nchar(SOPARC2023$periodBasedOnSequence))
                               

                 
[31] "yesno_snd"                           "snd_act_name"                        "other_act_snd"                       "yesno_spec"                          "spec_act_name"                      
[36] "other_act_spec"                      "comments"                            "park_scan_data_collection_complete"  "timestampPOSIX"                      "datePOSIX"                          
[41] "day"                                 "month"                               "weekend"                             "periodBasedOnSequence"               "dayNumberBasedOnSequence"           
[46] "studyStartDate"                      "studyDescription"                    "study_count"                  ]
                     "num_senior_tert"                     "num_senior_quat"                     "prim_act_name"                       "prim_act_other"                     
[36] "yesno_snd"                           "snd_act_name"                        "snd_act_other"                       "yesno_tert"                          "tert_act_name"                      
[41] "tert_act_other"                      "yesno_quat"                          "quat_act_name"                       "quat_act_other"                      "comments"                           
[46] "park_scan_data_collection_complete"  "timestampPOSIX"                      "datePOSIX"                           "day"                                 "month"                              
[51] "weekend"                             "periodBasedOnSequence"  
...
SOPARCAggregated <- SOPARCtoAggregate[,
                                      .("accessible" = ifelse(any(accessible %in% "Yes"), "Yes", "No"),
                                        ...
                                        "num_child_tert" = ceiling(mean(num_child_spec, na.rm = T)),
                                        "num_child_quat" = ceiling(mean(num_child_spec, na.rm = T)),
                                        "num_teen_prim"= ceiling(mean(num_teen_prim, na.rm = T)),
                                        "num_teen_snd" = ceiling(mean(num_teen_snd, na.rm = T)),
                                        "num_teen_tert" = ceiling(mean(num_teen_spec, na.rm = T)),
                                        "num_teen_quat" = ceiling(mean(num_teen_spec, na.rm = T)),
                                        "num_adult_prim" = ceiling(mean(num_adult_prim, na.rm = T)),
                                        "num_adult_snd" = ceiling(mean(num_adult_snd, na.rm = T)),
                                        "num_adult_tert" = ceiling(mean(num_adult_spec, na.rm = T)),
                                        "num_adult_quat" = ceiling(mean(num_adult_spec, na.rm = T)),
                                        "num_senior_prim" = ceiling(mean(num_senior_prim, na.rm = T)),
                                        "num_senior_snd" = ceiling(mean(num_senior_snd, na.rm = T)),
                                        "num_senior_tert" = ceiling(mean(num_senior_spec, na.rm = T)), 
                                        "num_senior_quat" = ceiling(mean(num_senior_spec, na.rm = T))), 
