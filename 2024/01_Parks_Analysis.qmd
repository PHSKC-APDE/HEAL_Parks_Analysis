...
parksMissingFromList <- unique(SOPARCObservations$park_name)[!(unique(SOPARCObservations$park_name)  %in% ParksMetaData$`Park Name`)]
parksMissingLongLat <- ParksMetaData[unique(`Park Name`) %in% unique(SOPARCObservations$park_name) & is.na(Longitude), `Park Name`]
...
The following parks are missing metadata. Importantly, longtitude and latitude
are needed to calculate populations `r parksMissingMetadata`
...
```{r}
SOPARCActivitiesAllYears <- rbind(SOPARCActivities, SOPARCActivities2022, SOPARCActivities2023)
```
...
DT <- SOPARCAggregated[order(park_name),.("Users" = sum(num_child_prim, num_child_snd, num_child_tert, num_teen_prim, num_teen_snd, num_teen_tert, num_adult_prim, num_adult_snd, num_adult_tert, num_senior_prim, num_senior_snd, num_senior_tert, na.rm = TRUE)), by = .("Park Name" = park_name)]
...
The daily average number of users observed in the park across all available data
