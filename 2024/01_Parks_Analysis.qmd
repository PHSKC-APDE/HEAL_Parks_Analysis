...
-   Copy edit note: I have been trying to change the language from “target area”
    to “scan area”
...
inconsistencies in 2022. Some may involve reclassifying entries, for example,
they would be "other" or fit in a pre identified category, and so look different
in the observations table. Others are impossible to reclassify, such as "1".
...
The program manager, Seth Schromen-Warren, provided corrections to data that
failed QA (described below). These corrections included a list of observations
requiring modification, and a list of observation that should be drooped from
the data. This was based on their knowledge of the parks and study procesees.
...
Pocket parks were classified as one park (with the target areas being the pocket
parks) in this year's data collection. Seth has provided a crosswalk for
unpacking these into multiple parks for analysis.
...
write.csv(RawSOPARCObservations,file = here(OUPUTS_LOCALE,"./QA/all_data.csv"))
write.csv(RawSOPARCObservations[park_scan_data_collection_complete != "Complete",] ,file = here(OUPUTS_LOCALE,"./QA/incomplete_data.csv"))
...
#| eval: false
#no sub areas were added in 2024. This code chunk was not updated for 2024.
...
QA status are saved in a csv file for review and manual correction where
necessary.
...
write.csv(parkStats, here("2024/outputs/QA/QA_report.csv"), row.names = F)
...
write.csv(SOPARCObservationsOK, here("2024/outputs/QA/QAPassSOPARCObservations.csv"), row.names = F)
write.csv(SOPARCObservationsToFix, here("2024/outputs/QA/QAFailSOPARCObservations.csv"), row.names = F)
...
attempts to insert the missing observations as blank entries in the parts of the
day that appear to be missing them.
...
The below steps assume that complete data are being used. Data that did not pass
QA are not utilized in the rest of the preperation or analysis.

```{r select analysis set to use}
#depreciated. Always use good data
...
In a later step we will use this table to update the study count to our parks
...
only need the last and the current years data, as the saved tables will include
all prior data.
...
SOPARCActivitiesAllYears <- rbind(SOPARCActivities[,.(record_id, record_id_aggregated, activity = tolower(activity), children, teen, adult, senior, studyDescription)], SOPARCActivities2022[,.(record_id, record_id_aggregated, activity = tolower(activity), children, teen, adult, senior, studyDescription)], SOPARCActivities2023[,.(record_id, record_id_aggregated, activity = tolower(activity), children, teen, adult, senior, studyDescription)])
```

### Fixing activities

Several activities can be reclassified for more accurate analysis. This is a
manual step, so information is provided below to assist.

Here is a list of activities presented to observers in 2023 and 2024. They may
also enter "other" and type a custom entry.
...
This year we focused on fixing errors in 2022 activities to help them align to
this list.

We also identified one 2024 activity where the observer entered "other" but did
not enter something into the box. this has been changed to "other".

Procedure:

For 2022 activities

Where an activity was not classified as "other" AND appears to match, but not
precisely conform to, the 2024 activities list, update both the activity table
and the observations table, keeping the activity in its observation step
(primary, secondary, or tertary activity).

If the activity was not classified as "other" AND does NOT appear to match the
2024 activities list, then, in the observations table, change the activity to
"other" and move the unmatched name to "other description" of the approprate
observation step. Leave unchanged in the activity table

```{r make all activities lower case}
SOPARCAnalysisSetAllYears[, `:=`(prim_act_name = tolower(prim_act_name), snd_act_name = tolower(snd_act_name), tert_act_name = tolower(tert_act_name), quat_act_name = tolower(quat_act_name))]

```

```{r implement corrections}



#an activity with "1"
SOPARCActivitiesAllYears <- SOPARCActivitiesAllYears[!(studyDescription == "2022 annual study round 1" & record_id == 131),] 
SOPARCAnalysisSetAllYears[studyDescription == "2022 annual study round 1" & record_id == 131,`:=`(num_senior_prim = NA, prim_act_name = NA)]

#NA
SOPARCActivitiesAllYears[record_id == 9553 & studyDescription == "2024 annual study", activity := "other"]
SOPARCAnalysisSetAllYears[record_id == 9553 & studyDescription == "2024 annual study", prim_act_other := "other"]

activityRenamer <- function(activitytarget, poornames, study, DT) {
  if("activity" %in% names(DT)) {
    DT[activity %in% poornames, activity := activitytarget]
    return(DT)
  }
  if("prim_act_name" %in% names(DT)) {
    DT[studyDescription %in% study & prim_act_name %in% poornames, prim_act_name := activitytarget]
    DT[studyDescription %in% study & snd_act_name %in% poornames, snd_act_name := activitytarget]
    DT[studyDescription %in% study & tert_act_name %in% poornames, tert_act_name := activitytarget]
    DT[studyDescription %in% study & quat_act_name %in% poornames, quat_act_name := activitytarget]
    return(DT)
  }
  stop("missuse of function, check code")
}

#make this loop (how do you store a vector in a cell?)
#corrections <- data.frame(act = tolower(activities), errors = "")

studies <- c("2022 annual study round 1", "2022 annual study round 2")

#fixing baseball/softball
baseball <- c("baseball")
SOPARCActivitiesAllYears <- activityRenamer("baseball/softball", baseball, studies, SOPARCActivitiesAllYears)
SOPARCAnalysisSetAllYears <- activityRenamer("baseball/softball", baseball, studies, SOPARCAnalysisSetAllYears)

#fix "bike riding"
biking <- c("riding bike", "ride bike", "rding bike","bicycle","bicycling", "bikng")
SOPARCActivitiesAllYears <- activityRenamer("bike riding", biking, studies, SOPARCActivitiesAllYears)
SOPARCAnalysisSetAllYears <- activityRenamer("bike riding", biking, studies, SOPARCAnalysisSetAllYears)

#fixing "Catch (any sport)"
catch <- c("plyg catch", "playing catch")
SOPARCActivitiesAllYears <- activityRenamer("catch (any sport)", catch, studies, SOPARCActivitiesAllYears)
SOPARCAnalysisSetAllYears <- activityRenamer("catch (any sport)", catch, studies, SOPARCAnalysisSetAllYears)

#fixing lying down
lyingdown <- c("laying")

#fixing running
running <- c("runnng","runing")


#fixing sitting
sitting <- c("siting","ppl sitting" )
SOPARCActivitiesAllYears <- activityRenamer("sitting", sitting, studies, SOPARCActivitiesAllYears)
SOPARCAnalysisSetAllYears <- activityRenamer("sitting", sitting, studies, SOPARCAnalysisSetAllYears)


#fix skating/skateboarding
skating <- c("skateboard ")


#fixing soccer
soccer <- c("drills soccer")
SOPARCActivitiesAllYears <- activityRenamer("soccer", soccer, studies, SOPARCActivitiesAllYears)
SOPARCAnalysisSetAllYears <- activityRenamer("soccer", soccer, studies, SOPARCAnalysisSetAllYears)

#fix standing
standing <- c("standaing")
SOPARCActivitiesAllYears <- activityRenamer("standing", standing, studies, SOPARCActivitiesAllYears)
SOPARCAnalysisSetAllYears <- activityRenamer("standing", standing, studies, SOPARCAnalysisSetAllYears)


#fix walking
walking <- c("walkers", "walkling", "walking path", "walker", "wakling")
SOPARCActivitiesAllYears <- activityRenamer("walking", walking, studies, SOPARCActivitiesAllYears)
SOPARCAnalysisSetAllYears <- activityRenamer("walking", walking, studies, SOPARCAnalysisSetAllYears)
...
```{r return activities in activity table to capital}
SOPARCActivitiesAllYears[, activity :=paste(toupper(substr(activity,1,1)), tolower(substr(activity, 2, nchar(activity))), sep = "") ]

```

After making corrections, here are activities in our data that are not in the
above list. These should be manually reviewed for possible further correction.

`r unique(SOPARCActivitiesAllYears$activity)[!(unique(SOPARCActivitiesAllYears$activity) %in% activities)]`
...
#save results
write.csv(SOPARCtoAggregate, here("2024/outputs/data-metadata/SOPARCAnalysisSetAllYears.csv"), row.names = FALSE)
write.csv(SOPARCAggregated, here("2024/outputs/data-metadata/SOPARCAnalysisSetAggregatedPeriodsAllYears.csv"), row.names = FALSE)

write.csv(SOPARCActivitiesAllYears, here("2024/outputs/data-metadata/SOPARCActivitiesAllYears.csv"), row.names = FALSE)


write.csv(merge(SOPARCActivitiesAllYears,SOPARCtoAggregate[,.(park_name, record_id, period, tar_area, datePOSIX)], by.x = "record_id_aggregated", by.y = "record_id"), here("2024/outputs/data-metadata/SOPARCActivitiesAllYearsExpanded.csv"), row.names = FALSE)
...
Several of the metrics rely on counts of people observed. The underlying data
are of people observed within an observation period (e.g. "morning1") and target
...
The below analyses include parks studied from 2022 to present unless otherwise
indicated. For these analyses all available data are used. This means that if
the same park has been studied for multiple years, all years are included.

## Park Utilization

### number of park users

The total number of users observed for a given park across all available data.
...
#pulled from 2023, might not be appropriate at all
...
-   Due to the study design, user counts are subject to both over and under
    counting
-   Does not adjust for number of studies or days of park observation. Therefore
    more heavily studied parks will tend towards higher counts.

### daily average number of park users

The average number of users observed in the park per day across all available
data.
...
#cleanup old images of this chart
if(file.exists(here("./2024/outputs/tables/avg_user/"))) {
  unlink(here("./2024/outputs/tables/avg_user/"), recursive = T)
}
dir.create(here("./2024/outputs/tables/avg_user"), recursive = T)
write.csv(DT, here("2024/outputs/tables/avg_user/avg_user.csv"), row.names = FALSE)
FT <- DT[, .(`Park Name`,"Users" = round(Users,0))] %>% regulartable() %>% autofit()

read_docx() %>%
body_add_flextable(FT) %>%
print(target = paste0("./outputs/tables/avg_user/avg_user.docx"))  
...
The median number of users observed in the park in a given day across all
available data.

```{r median number of daily users}
#| output: true

DT <- SOPARCAggregated[order(park_name),.("Users" = (sum(num_child_prim, num_child_snd, num_child_tert, num_child_quat, num_teen_prim, num_teen_snd, num_teen_tert, num_teen_quat, num_adult_prim, num_adult_snd, num_adult_tert, num_adult_quat, num_senior_prim, num_senior_snd, num_senior_tert, num_senior_quat, na.rm = TRUE))), by = .("Park Name" = park_name, datePOSIX)][, .(Users = round(median(Users), 0)), by = "Park Name"]

WB$add_worksheet("Daily User Median")
WB$add_data(sheet = "Daily User Median", DT)


wsName <- "Daily User Median"
...
#save charts as individual files
#cleanup old images of this chart
if(file.exists(here("./2024/outputs/tables/med_user/"))) {
  unlink(here("./2024/outputs/tables/med_user/"), recursive = T)
}
dir.create(here("./2024/outputs/tables/med_user"), recursive = T)
write.csv(DT, here("2024/outputs/tables/med_user/med_user.csv"), row.names = FALSE)
FT <- DT[, .(`Park Name`,"Users" = round(Users,0))] %>% regulartable() %>% autofit()

read_docx() %>%
body_add_flextable(FT) %>%
print(target = paste0("./outputs/tables/med_user/med_user.docx"))  
...
summed across across all target areas. This is precalculated in the
SOPARCAggregated table.
...
The median people within a given time period and day is defined as the median
number of people observed in a time period across all days of a given park.
...
```{r median park users by period, fig.cap = "Median Park Users By Period"}
#| output: true

DT <- SOPARCAggregated[order(park_name),.("Users" = sum(num_child_prim, num_child_snd, num_child_tert, num_child_quat, num_teen_prim, num_teen_snd, num_teen_tert, num_teen_quat, num_adult_prim, num_adult_snd, num_adult_tert, num_adult_quat, num_senior_prim, num_senior_snd, num_senior_tert, num_senior_quat, na.rm = TRUE)), by = .("Park Name" = park_name, "Time Period" = period, "Date" = datePOSIX)][, .(Users = round(median(Users),0)), by = c("Park Name", "Time Period")]


DT$`Time Period` <- factor(DT$`Time Period`, level = c("Morning","Lunch","Afternoon","Evening"))

WB$add_worksheet("Med Users x Period")
WB$add_data(sheet = "Med Users x Period", DT)

wsName <- "Med Users x Period"
...
#save charts as individual files
#cleanup old images of this chart
if(file.exists(here("./2024/outputs/charts/med_users_x_period/"))) {
  unlink(here("./2024/outputs/charts/med_users_x_period/"), recursive = T)
}
dir.create(here("./2024/outputs/charts/med_users_x_period"), recursive = T)
...
for(park in unique(SOPARCAggregated$park_name)) {
  ...
  PTExport <- ggplot(DT[`Park Name` %in% park], (aes(x = `Time Period`, y = `Users`))) +
  geom_col()  +
  #labs(title = paste0(park,"\n", cityName,"\n", "2024"), subtitle = paste("Median Users Per Period")) +
  labs(title = paste0(park," ", cityName," ", "2024"), subtitle = paste("Median Users Per Period")) +
  geom_label(aes(label = `Users`, colour = "#000000", vjust =  ifelse(`Users` > tooLow, 1.5, 0) )) +
  scale_color_manual(values=c("#000000")) +
  scale_y_continuous(limits=c(0,topOfY), breaks = integer_breaks()) +
  theme(plot.title = element_text(size = 8),
  ...
  ggsave(paste0(park,"_med_users_x_period.pdf"), plot = PTExport, path = paste0("./outputs/charts/med_users_x_period"),width = CHARTWIDTH, height = CHARTHEIGHT)
  ggsave(paste0(park,"_med_users_x_period.png"), plot = PTExport, path = paste0("./outputs/charts/med_users_x_period"),width = CHARTWIDTH, height = CHARTHEIGHT)
...
if(!SUPPRESS_TABLES){
  kbl(DT,
      caption = "Media Number of Users Per Time Period",
      booktabs = TRUE,
      longtable = TRUE) %>% kable_styling(latex_options = c("striped", "repeat_header", "hold_position"))
}
...
```{r rate of daily average park use by period, fig.cap="Rate Of Daily Average Park Use By Period"}
#| output: true
...
```{r daily average park users by age, fig.cap="Average Park Users By Age"}
#| output: true
...
Computationally this measure is similar to the median users by time period
above.

```{r median park users by age, fig.cap="Average Park Users By Age"}
#| output: true

DT <- SOPARCAggregated[order(park_name), .( "Age Group" = rep(c("child","teen","adult","senior")), 
                                           "Users" = c(sum(num_child_prim, 
                                                                num_child_snd,
                                                                num_child_tert,
                                                                num_child_quat,
                                                                na.rm = TRUE),
                                                            ...
                                                            sum(num_senior_prim, 
                                                                ...
                                                                num_senior_quat,
                                                                na.rm = TRUE))), 
                       by = .("Park Name" = park_name, datePOSIX)][,.(Users = round(median(Users),0)), by = c("Park Name", "Age Group")]

DT$`Age Group` <- factor(DT$`Age Group`, level = c("child", "teen", "adult", "senior"))

WB$add_worksheet("Med Users x Age")
WB$add_data(sheet = "Med Users x Age", DT)

wsName <- "Med Users x Age"
...
#save charts as individual files
#cleanup old images of this chart
if(file.exists(here("./2024/outputs/charts/med_users_x_age/"))) {
  unlink(here("./2024/outputs/charts/med_users_x_age/"), recursive = T)
}
dir.create(here("./2024/outputs/charts/med_users_x_age"), recursive = T)
...
for(park in unique(SOPARCAggregated$park_name)) {
  ...
  PTExport <- ggplot(DT[`Park Name` %in% park,], (aes(x = `Age Group`, y = `Users`))) +
  geom_col()  +
  labs(title = paste0(park," ", cityName," ", "2024"), subtitle = paste("Median Users Per Age")) +
  geom_label(aes(label = `Users`, colour = "#000000", vjust =  ifelse(`Users` > tooLow, 1.5, 0))) +
  scale_color_manual(values=c("#000000")) +
  scale_y_continuous(limits=c(0,topOfY), breaks = integer_breaks()) +
  theme(plot.title = element_text(size = 8),
  ...
  ggsave(paste0(park,"_med_users_x_age.pdf"), plot = PTExport, path = paste0("./outputs/charts/med_users_x_age"),width = CHARTWIDTH, height = CHARTHEIGHT)
  ggsave(paste0(park,"_med_users_x_age.png"), plot = PTExport, path = paste0("./outputs/charts/med_users_x_age"),width = CHARTWIDTH, height = CHARTHEIGHT)
}

if(!SUPPRESS_TABLES){
  kbl(DT,
    caption = "Median Number of Users Per Age Group",
    booktabs = TRUE,
    longtable = TRUE) %>% kable_styling(latex_options = c("striped", "repeat_header", "hold_position"))
...
DT <- SOPARCAggregated[order(park_name),.("Users" = sum(num_child_prim, num_child_snd, num_child_tert, num_child_quat, num_teen_prim, num_teen_snd, num_teen_tert, num_teen_quat, num_adult_prim, num_adult_snd, num_adult_tert, num_adult_quat, num_senior_prim, num_senior_snd, num_senior_tert, num_senior_quat, na.rm = TRUE)/(3* unique(study_count) )), by = .("Park Name" = park_name)]
...
#save charts as individual files
#cleanup old images of this chart
if(file.exists(here("./2024/outputs/tables/ratio_use_catch/"))) {
  unlink(here("./2024/outputs/tables/ratio_use_catch/"), recursive = T)
}
dir.create(here("./2024/outputs/tables/ratio_use_catch"), recursive = T)
  write.csv(DT, here("2024/outputs/tables/ratio_use_catch/ratio_use_catch.csv"), row.names = FALSE)
...
-   Note, includes ALL years. Due to the varying number of times a park has been
    observed and is not adjusted for how many times a park has been observed
...
-   If an individual is observed doing a different activity in the second half
    ...
    activities, and so not averaged. This can result in slightly higher counts
...
total number of users observed in the park throughout all days and studies
observed.
...
For each park, the total number of periods the activity was observed is divided
by 24 \* (the number of times studied), which is the total number of
observations periods possible for any particular activity.

```{r Rate of Activity Observed}
#| output: true

#construct an activity table from the activity list
DT <- merge(SOPARCActivities[,.(record_id_aggregated,activity)], SOPARCtoAggregate[record_id != "X",.("record_id" = as.numeric(record_id), park_name, periodBasedOnSequence,dayNumberBasedOnSequence, studyDescription, study_count)], all.x = T, by.x = "record_id_aggregated", by.y = "record_id")

DT <- DT[,.N, by = .(park_name,activity,periodBasedOnSequence,dayNumberBasedOnSequence, studyDescription, study_count)]

DT <- DT[order(park_name, activity), .("Rate" = .N/ (24 * unique(study_count))), by = .("Park Name" = park_name, "Activity" = activity)]
...
#save charts as individual files
#cleanup old images of this chart
if(file.exists(here("./2024/outputs/tables/rate_activity_observed/"))) {
  unlink(here("./2024/outputs/tables/rate_activity_observed/"), recursive = T)
}
dir.create(here("./2024/outputs/tables/rate_activity_observed"), recursive = T)
...
-   These rates are mostly independent of each other and do not have an additive
    meaning. This is because multiple activities may be observed in a single
    observation period. E.g. "walking" may be observed in all 24 periods, and so
    have an observation rate of 100%, and "sitting" may be observed in 6 periods
    and so have a rate of 25% for the same park.
...
-   SOPARCAnalysisSetAggregatedPeriodsAllYears.csv (analysis set with
    observations aggregated by period, the level of analysis most commonly used)
...
-   The sub area aggregation code was not updated this year because no sub areas
    were observed.

## REDCap maintanence

Following closure of data collection the REDCap project should be moved into
"analysis/cleanup". It is recommended to keep it in this stage until finished
with any future studies and analyses in this line of work, e.g. if planing to
use the data from this study in the future, it is good to keep this REDCap
project in analysis mode for review. There is an additional "completed" status,
which should generally only be used when completely finished with the body of
work. This status makes the project largely inaccessible. Notably, changing the
project to "complete" status would also break this script (or require it to be
fed a different data source rather than pulling from the REDCap API.)
...
Park Observations: title_of_study" The project used for this analysis has been
renamed to: Public Health - Seattle & King County Park Observations: 2024 Annual
Study
...
-   Include additional testing and possibly consultation with ITHS REDCap staff
    ...
    observers.
-   A participant was able to choose "other" activity but not enter something in
    the other description. This shouldn't have been possible. Fix the instrument
    validation to prevent this. (2024 study, record_id 15651 in raw, 9553 in
    prepared)

## Documentation

Documentation for this project is available in [git
hub](https://github.com/PHSKC-APDE/HEAL_Parks_Analysis) and in the [project
sharepoint
