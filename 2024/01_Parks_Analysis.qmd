...
write.csv(RawSOPARCObservations,here(paste0(DATA_LOCALE,"REDCapSOPARCObservations.csv")), row.names = FALSE)

```

```{r apply manual corrections from Seth}
DataToChange <- read_xlsx(here(INPUTS_LOCALE,"2024DatetoChange.xlsx"))
DataToChange$time_period <- factor(DataToChange$time_period, c("1","2","3","4"),c("Morning","Lunch","Afternoon", "Evening") )
DataToChange$time_period_half <- factor(DataToChange$time_period, c("1","2"),c("first observation","second observation") )
parks <- c("Annex Park",
  "Arbor Lake Park",
...
DataToChange$park_name <- factor(DataToChange$park_name,1:55,parks )

DataToChange$accessible <- factor(DataToChange$accessible, c("1","0"),c("Yes","No") )
DataToChange$lit <- factor(DataToChange$lit, c("1","0"),c("Yes","No") )
DataToChange$occupied <- factor(DataToChange$occupied, c("1","0"),c("Yes","No") )
DataToChange$usable <- factor(DataToChange$usable, c("1","0"),c("Yes","No") )
DataToChange$supervised <- factor(DataToChange$supervised, c("1","0"),c("Yes","No") )
DataToChange$organized <- factor(DataToChange$organized, c("1","0"),c("Yes","No") )
DataToChange$equipped <- factor(DataToChange$equipped, c("1","0"),c("Yes","No") )

DataToChange$yesno_snd <- factor(DataToChange$yesno_snd, c("1","0"),c("Yes","No") )
DataToChange$yesno_tert <- factor(DataToChange$yesno_tert, c("1","0"),c("Yes","No") )
DataToChange$yesno_quat <- factor(DataToChange$yesno_quat, c("1","0"),c("Yes","No") )
...
DataToChange$prim_act_name <- factor(DataToChange$prim_act_name,1:25,activities )
DataToChange$snd_act_name <- factor(DataToChange$snd_act_name,1:25,activities )
DataToChange$tert_act_name <- factor(DataToChange$tert_act_name,1:25,activities )
DataToChange$quat_act_name <- factor(DataToChange$quat_act_name,1:25,activities )
...
RawSOPARCObservations$num_teen_uat <- as.numeric(RawSOPARCObservations$num_teen_quat)
...
SOPARCObservations[record_id == 4706, "tar_area"] <- 5
...
if( !(file.exists(here("2023/local_only/SOPARCActivities.csv"))) | REBUILDACTIVITYTABLE){
...
SOPARCActivities <- data.table()
  ...
  for(id in SOPARCObservations[prim_act_name != "",]$record_id) {
    ...
    if(SOPARCObservations[record_id == id,]$yesno_snd == "Yes") {
    ...
    if(SOPARCObservations[record_id == id,]$yesno_spec == "Yes") {
  ...
  write.csv(SOPARCActivities, here("2023/local_only/SOPARCActivities.csv"), row.names = FALSE)
  write.csv(SOPARCActivities, here("2023/outputs/data-metadata/SOPARCActivities.csv"), row.names = FALSE)
} else {
  SOPARCActivities <-  fread(here("2023/local_only/SOPARCActivities.csv"))
}
```
...
write.csv(SOPARCActivities, here("2023/outputs/data-metadata/Park Activities.csv"), row.names = F)
...
day that appeaer to be missing them. It identifies part of day by looking at
each data collection period and inserting observations into the underweighted
period(s) until 8 exists for each target area.
...
zip, neighborhood, tract, equity score, image status, planned park change notes,
and general notes for each park will be appended to the analysis table.
...
skip <- FALSE
if(file.exists(here("2023/local_only/Park Populations.csv"))) {
  ParkPopulationTable <- fread(here("2023/local_only/Park Populations.csv"))
...
if(!skip | RECALCULATEPOPULATIONS) {
  ...
  ParkGeos <- ParksMetaData[,.("park" = `Park Name`, "long" = Longitude, "lat" = Latitude)] #create a prototype data frame for holding our geometry. This has the raw lat long data
  ...
  if(file.exists(here("2023/inputs/KCPops.csv"))) {
    KCPops <- read.csv(here("2023/inputs/KCPops.csv"))
  } else {
    KCPops <- get_population(geo_type = "blk", kingco = T, year = 2022)
    write.csv(x = KCPops, here("2023/inputs/KCPops.csv"), row.names = F)
  }
  
  #Create data frame for final results
  ParkPopulationTable <- data.table("parkNameFull" = ParksMetaData$`Park Name`, "censusYear" = 2022, "populationHalfMile" = 0)
  ...
  write.csv(ParkPopulationTable ,here("2023/outputs/data-metadata/Park Populations.csv"), row.names = F)
  write.csv(ParkPopulationTable ,here("2023/local_only/Park Populations.csv"), row.names = F)
}
```
...
is studied, a new entry is created. Each entry should includ the park name, the
date the study staretd, and a brief description of the study. We generate and
save a study tracker that covers 2022 and 2023 studies.
...
For each observation we need to assign a time period to that observation.
Because time stamps and the actual time of data collection may not align, the
assignment of the time period requires some assumptions and may effect results.

```{r period assignment based on timestamp}
#| eval: false

#this was an earlier attempt that is not used in teh final analysis
...
Our primary approach to assigning time periods relies on the assumption that
timestamps are in order for each target area (e.g. the first observation of park
A area 1 is always "morning1", the second, "morning2" and so on.)

In turn, with this approach, we assign the sequence of the day such that the
first morning1 is day 1, the second is day2, and so on.

We assume that there are the correct number of observation periods such that 1
can be assigned to each period, and 8 to each day.

For each park, there should be 3 days of observation. We will detect this by
extracting the date per park and putting observations in order of day "1" "2"
"3".
...
for(park in unique(SOPARCAnalysisSet$park_name)) {
  ...
  for(dateIndex in 1:length(datesToMatch)) {
    ...
    for(taIndex in 1:taMax) {
      if(nrow(SOPARCAnalysisSet[datePOSIX == date & park_name == park & tar_area == taIndex,][order(timestampPOSIX),]) != 8) {errorCatch[nrow(errorCatch)+1, "message"]<-  paste0(park," on day ", as.POSIXct.Date(date), " for target area ",taIndex, " has ", nrow(SOPARCAnalysisSet[datePOSIX == date & park_name == park & tar_area == taIndex,][order(timestampPOSIX),]) , " observations, should have 8")}

      SOPARCAnalysisSet[datePOSIX == date & park_name == park & tar_area == taIndex,][order(timestampPOSIX), ]$periodBasedOnSequence <- sequenceLabels
      #SOPARCAnalysisSet[datePOSIX == date & park_name == park & tar_area == taIndex,][order(timestampPOSIX), ]
    }
  }
}
...
-   All all parks have the expected number of period observations (e.g. if a
    park has 4 target areas, it should have 12 morning1 observations).
-   All observations assigned to a particular day-count (e.g. "1") actually are
    on the same date (this assumption may need to be relaxed or data corrected
    before all data pass QA).
...
SOPARCtoAggregate <- merge(SOPARCtoAggregate, ParkPopulationTable, by.x = "park_name_full", by.y = "parkNameFull")
SOPARCtoAggregate$studyStartDate <- "2023-07-01"
SOPARCtoAggregate$studyDescription <- "2023 annual study"
...
SOPARCAggregated <- merge(SOPARCAggregated, ParksMetaData[,.(`REDCap Name`,
                                                  ...
                                                  `Planned park changes?`,
                                                  Notes)], by.x = "park_name", by.y = "REDCap Name", all.x = T)
SOPARCAggregated <- merge(SOPARCAggregated, ParkPopulationTable, by.x = "park_name_full", by.y = "parkNameFull")
SOPARCAggregated$studyStartDate <- "2023-07-01"
SOPARCAggregated$studyDescription <- "2023 annual study"
